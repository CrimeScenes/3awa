...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        -- Predict future position using velocity and prediction multiplier
...
local function TriggerBotAction()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            ...
            if getgenv().TriggerBot.Settings.Preds.EnableLegitPrediction then
            ...
            if predictedPosition and isWithinBoxFOV(predictedPosition) then
                ...
                if closestPart then
                    ...
                    if distance <= getgenv().TriggerBot.Settings.FiringDistance then
                        local minDelay = getgenv().TriggerBot.Settings.TriggerDelay.Min or 0.01
                        local maxDelay = getgenv().TriggerBot.Settings.TriggerDelay.Max or 0.1
                        local currentDelay = math.random(minDelay, maxDelay)
